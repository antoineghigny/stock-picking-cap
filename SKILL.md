...
### Step 11: Expected Value and Asymmetry

Before finalising, convert the qualitative analysis into a probabilistic decision framework. This step is mandatory — a CAP gap without probability-weighted scenarios is an incomplete analysis.

**Method:**

1. Define 3-4 scenarios covering the range of possible outcomes, anchored by your CAP estimate and the market's implied CAP:
   - **Bull**: Your Real CAP estimate is approximately correct → upside capture
   - **Base**: Most likely outcome (CAP somewhere between Real and Implied)
   - **Mild Bear**: CAP closer to Implied; moat erodes faster than expected
   - **Full Bear**: Market is correct; CAP is at or below Implied (permanent capital loss scenario)

2. For each scenario, estimate:
   - Probability weight (must sum to 100%)
   - EPS CAGR over 5 years
   - Terminal P/E (consistent with that scenario's moat trajectory)
   - 5-year total return (CAGR + multiple expansion/compression)
   - Rationale: one-sentence narrative justifying the numbers

3. Calculate probability-weighted expected return:
   ```
   E[Return] = Σ(P_i × Return_i)
   ```

4. Calculate asymmetry:
   ```
   Upside/Downside Ratio = Best Case Return ÷ |Worst Case Return|
   ```
   - Ratio > 3x: favourable asymmetry
   - Ratio 1-3x: moderate
   - Ratio < 1x: unfavourable (downside dominates)

5. Estimate permanent capital loss probability:
   - "Permanent capital loss" = total real return < -30% over 5 years (including dividends)
   - Assign based on: moat width confidence, balance sheet strength, binary event risk (patent cliff, regulatory, pipeline failure)
   - Cross-check: has this company's equity ever lost 30%+ real and stayed there for 5+ years?

6. Identify catalysts required for CAP gap to close:
   - List 2-4 specific, observable events that would narrow the gap between Real and Implied CAP
   - For each: timeline, what "good enough" looks like, probability of success
   - If no catalysts exist within 12-18 months, the gap may persist (value trap risk)

7. Derive position sizing guidance:
   - Use expected value, confidence, and asymmetry to suggest a max portfolio allocation
   - Adjust for liquidity, correlation to existing holdings, and personal circle of competence

**Decision framework:**

| E[Return] vs Benchmark | Permanent Loss Risk | Asymmetry | Suggested Max Allocation |
|------------------------|-------------------|-----------|-------------------------|
| > +5%/yr alpha | < 10% | > 3x | 10-15% (high conviction) |
| +2-5%/yr alpha | 10-20% | 1-3x | 5-10% (moderate) |
| < +2%/yr alpha | > 20% | < 1x | 0-3% (skip or token) |

**Final check:** *"Does the probability-weighted expected return, after accounting for permanent capital loss risk, materially exceed the index return? If yes, only then should this position be considered for the active portion of the portfolio."*
...
## 11. Expected Value and Asymmetry

### Scenario Weights
| Scenario | Probability | EPS CAGR (5yr) | Terminal P/E | 5yr Total Return | Rationale |
|----------|-------------|----------------|--------------|------------------|-----------|
| Bull (Real CAP correct) | XX% | XX% | XXx | +XX% | [1-sentence narrative] |
| Base (most likely) | XX% | XX% | XXx | +XX% | [1-sentence narrative] |
| Mild bear | XX% | XX% | XXx | +XX% | [1-sentence narrative] |
| Full bear (market right) | XX% | XX% | XXx | -XX% | [1-sentence narrative] |

### Expected Return
- Probability-weighted 5yr return: XX% → ~XX% CAGR
- vs Broad Index expected return: ~XX% CAGR
- Probability-weighted alpha: ~XX%/year

### Asymmetry
- Best case (5yr): +XX% | Worst case (5yr): -XX%
- Upside/Downside ratio: X.Xx
- Permanent capital loss probability (>-30% real): XX%

### Catalysts Required for Gap Closure
1. [Catalyst] — [timeline] — [what "good enough" looks like] — [estimated probability]
2. [Catalyst] — [timeline] — [what "good enough" looks like] — [estimated probability]

### Position Sizing Guide
- Suggested max allocation: [X]% of active portfolio
- Rationale: [one sentence linking expected value, confidence, and asymmetry]
```
...
9. **Expected value before conviction**: Do not present a verdict without Step 11 probability-weighting. A wide CAP gap is not a buy signal — it is a hypothesis that must survive scenario weighting. The asymmetry calculation (Section 11) is what separates analysis from advocacy.
10. **Perma-capital loss is the real risk**: A stock can be undervalued on CAP for years and still lose money if the catalyst never arrives. Step 11 forces explicit consideration of this outcome. If the full-bear scenario probability exceeds 15-20%, position sizing must reflect it regardless of how wide the CAP gap appears.
